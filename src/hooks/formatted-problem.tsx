...
export const useFormatedProblem = (problemText: string) => {
  ...
  useEffect(() => {
    ...
    for (const i of inline) {
      const a = KaTeX.renderToString(i.substring(1, i.length - 1), { displayMode: false, throwOnError: false })
      text = text.replace(i, a)
    }

    for (const b of block) {
      const a = KaTeX.renderToString(b.substring(2, b.length - 2), { displayMode: true, throwOnError: false })
